...
def default_outputs():
    return (
        LiteralOutput(
            'success',
            'Success',
            abstract='True if the metric has been successfully calculated. If false please check the log files',
            data_type='string')
    )


class CorrelateField(Process):
    """ CorrelateField process """
    def __init__(self):
        inputs = [
